...
function consultarCorrelativoParaCarnet(idIntegrante) {
    ...
    $.ajax({
        ...
        success: function(response){
                ...
                var idInterno = datos[1];
                var identidad = datos[4];
                var correlativo = datos[0];
                var tel1 = datos[2];
                var tel2 = datos[3];
            sendData(idInterno,identidad,correlativo,tel1,tel2);
...
function sendData(idInterno,identidad,correlativo,tel1,tel2)
{
    ...
    checkPrinterStatus( function (text){
        if (text == "Ready to Print")
        {
            ...
            selected_printer.send(n1+promocion1+n2+nombreNuevo+n3+idInterno+n4+correlativo+n5+tel2+n6+identidad+n7+tel1+n8, printComplete, printerError);
            //$('#formularioRegistro')[0].reset();
        }
        else
        {
